...
Run `docker-compose up smart_backend` in one terminal window.
Inside `frontend/`, run `npm run serve`

This uses the webpack-dev-server as the main server, and proxy requests that are not assets generated by webpack to the backend server.
